...
const Checkout = () => {
  ...
  const { accountAddress, tokenContract, busdContract, opcoStoreContract } =
    blockchain;
  ...
  const approveBusd = async () => {
    try {
      ...
      busdContract.on("Approval", (event) => {
        setCheckApprovedToken(false);
        setLoading(false);
        Swal.fire({
          title: 'Success',
    ...
    if (
      ...
      if (checkoutFull) {
        try {
          ...
          opcoStoreContract.on('order', (result) => {
            try {
      ...
      {success ? (
        ...
        <div className="max-w-xl px-8 py-0 mx-auto my-2.5">
          <div className="flex flex-col items-center justify-between mb-4">
            ...
            <button className="bg-black text-white uppercase text-xs font-semibold py-4 px-6 rounded focus:outline-none" onClick={resetAddress}>
              Clear Checkout
            </button>
          </div>
          <p>Please fill in the shipping details.</p>
          <hr />
          <div className="grid gap-4">
            <div className="grid gap-4">
              <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                <span className="text-gray-500 text-md uppercase">Name</span>
                <input
                  className="font-semibold text-md w-full focus:outline-none"
                  type="text"
            ...
            <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
              <span className="text-gray-500 text-md uppercase">Direction</span>
              <input
                className="font-semibold text-md w-full focus:outline-none"
                type="text"
            ...
            <div className="grid gap-4">
              <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                <span className="text-gray-500 text-md uppercase">Country</span>
                <input
                  className="font-semibold text-md w-full focus:outline-none"
                  type="text"
              ...
              <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                <span className="text-gray-500 text-md uppercase">Phone</span>
                <PhoneInput
                  className="font-semibold text-md w-full focus:outline-none"
                  value={Phone}
                  onChange={setPhone}
              ...
              <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                <span className="text-gray-500 text-md uppercase">Zip code</span>
                <input
                  className="font-semibold text-md w-full focus:outline-none"
                  type="text"
              ...
              <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                <span className="text-gray-500 text-md uppercase">City</span>
                <input
                  className="font-semibold text-md w-full focus:outline-none"
                  type="text"
                  value={Address.city}
              ...
              <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                <span className="text-gray-500 text-md uppercase">Estate</span>
                <input
                  className="font-semibold text-md w-full focus:outline-none"
                  type="text"
                  value={Address.state}
              ...
              <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                <span className="text-gray-500 text-md uppercase">Token</span>
                <select
                  value={token}
                  onChange={(e) => {
              ...
              <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                <span className="text-gray-500 text-md uppercase">SubTotal</span>
            ...
            {cartItems.map((item, index) => (
              <div className="grid gap-4" key={index}>
                <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                  <span className="text-gray-500 text-md uppercase">Product</span>
                  <div className="font-semibold">{item.name}</div>
                </label>
                <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                  <span className="text-gray-500 text-md uppercase">Option</span>
                  <div className="font-semibold">{item.option}</div>
                </label>
                <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                  <span className="text-gray-500 text-md uppercase">Discount</span>
                  <div className="font-semibold">{token === Api.TOKEN_NAME ? item.discount : 0}%</div>
                </label>
                <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                  <span className="text-gray-500 text-md uppercase">Price</span>
                  <div className="font-semibold">
                ...
                <label className="w-full flex flex-col border-2 p-2 rounded-3xl">
                  <span className="text-gray-500 text-md uppercase">Amount</span>
          ...
          <div className="flex justify-center">
            <button
              onClick={handleOrden}
              className="bg-black text-white uppercase text-xs font-semibold py-4 px-6 rounded-sm focus:outline-none"
            >
              {accountAddress
                ? token === Api.TOKEN_NAME
